...
const apiVersion = 'v3'
const resource = 'entities/opportunities'
...
const actions = {
      ...
      new ApiClient(url, {apiVersion}).get()
        .then(res => {
          if (res.data.msg) context.commit(`global/${types.SET_ERROR}`, res.data.msg, {root: true})
            ...
            context.commit(types.entity.opportunities.get, JSON.parse(res.data.data))
      ...
      new ApiClient(resource, {apiVersion}).show(id)
        .then(res => {
          if (res.data.msg) context.commit(`global/${types.SET_ERROR}`, res.data.msg, {root: true})
            ...
            context.commit(types.entity.opportunities.getone, JSON.parse(res.data.data))
  ...
  query: async (context, query) => {
    query
    ? context.commit(types.entity.opportunities.query, query)
    : context.commit(types.entity.opportunities.query, null)
  },
  queryHistory: async (context, track) => {
    if (track) context.commit(types.entity.opportunities.queryHistory, track)
  },
  editID: async (context, editID) => {
    context.commit(types.entity.opportunities.editID, editID);
  },
  editing: async (context, editing) => {
    context.commit(types.entity.opportunities.editing, editing);
  },
  opportunity: async (context, opportunity) => {
    context.commit(types.entity.opportunities.getone, opportunity)
  },
  page: async (context, page) => {
    context.commit(types.entity.opportunities.page, page)
  },
};
