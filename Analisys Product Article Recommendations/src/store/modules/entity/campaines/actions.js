...
const apiVersion = 'v3'
const resource = 'entities/campaines'
...
const actions = {
      ...
      new ApiClient(url, {apiVersion}).get()
        .then(res => {
          if (res.data.msg) context.commit(`global/${types.SET_ERROR}`, res.data.msg, {root: true})
            ...
            context.commit(types.entity.campaines.get, JSON.parse(res.data.data))
      ...
      new ApiClient(resource, {apiVersion}).show(id)
        .then(res => {
          if (res.data.msg) context.commit(`global/${types.SET_ERROR}`, res.data.msg, {root: true})
            ...
            context.commit(types.entity.campaines.getone, JSON.parse(res.data.data))
  ...
  query: async (context, query) => {
    query
    ? context.commit(types.entity.campaines.query, query)
    : context.commit(types.entity.campaines.query, null)
  },
  queryHistory: async (context, track) => {
    if (track) context.commit(types.entity.campaines.queryHistory, track)
  },
  editID: async (context, editID) => {
    context.commit(types.entity.campaines.editID, editID);
  },
  editing: async (context, editing) => {
    context.commit(types.entity.campaines.editing, editing);
  },
  campaine: async (context, campaine) => {
    context.commit(types.entity.campaines.getone, campaine)
  },
  page: async (context, page) => {
    context.commit(types.entity.campaines.page, page)
  },
};
